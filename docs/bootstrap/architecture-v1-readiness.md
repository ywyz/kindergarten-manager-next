...
### 下一项：I5 Word 导出（实施规格已准备，未实施）

[I5 Word 导出实施规格](../specs/word-export-implementation.md) 已于 2026-09-25 完成规格准备：基于既有 Contract、最小实施规格、Word 验证计划/原型结果与 I3/I4 读取面的只读现状核对，收敛了 API 边界、导出读取面、模板映射、生成层与下载边界、错误语义、审计核对、分层验证矩阵，并给出四片实施切片（读取与映射纯逻辑 / docx 生成 / API 下载闭环 / 隔离 Word 与浏览器验收）。文件上限、临时文件清理、生产生成库、模板资产入库、导出审计等保持为**提案**待确认；**本轮仅新增规格文档，无任何导出实现、依赖、模板资产或入口，也不构成任何切片授权**。

**2026-09-25 定向修订（规格层面，未实施）**：按 [I5 规格审核](i5-spec-review-2026-09-25.md) 完成修订——修正 `no_plan` 清空已确认人工内容、跨学期周号排序/去重、服务端“未包含最新变化”判定与提示承载三处缺陷；日计划缺项确认的跨请求绑定（规格 §11.8，建议评估 B）与确认后日历变化的导出布局（§11.9，不代选）列为待用户决定，片 1 依赖 §11.7/§11.8。仍为规格文档：未实施、未授权任何实现切片，未运行测试、数据库、服务、浏览器或 LibreOffice。
