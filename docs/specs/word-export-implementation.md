...
状态：**规格已按 2026-09-25 审核（[审核报告](../bootstrap/i5-spec-review-2026-09-25.md)）完成定向修订：F1–F3 缺陷已修正；两项语义已由用户决定并落入本规格——§11.7 零上课日周取方案①、§11.8 日计划缺项确认取方案 B（绑定已展示的版本/事实）；§11.9（确认后日历变化布局）仍待用户决定；规格仍未实施、未授权实施。** 本文把既有已确认的“手工日计划 / 已确认周计划 → 固定 Word 模板导出”规则收敛为可逐片授权实施的 I5 规格；只做规格、现状核对与实施切片设计，不含任何业务代码、schema/迁移、依赖安装或服务启动。
