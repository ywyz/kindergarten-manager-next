...
状态：**规格已按 2026-09-25 审核（[审核报告](../bootstrap/i5-spec-review-2026-09-25.md)）完成定向修订：F1–F3 缺陷已修正，两项语义（缺项确认跨请求绑定 §11.8、确认后日历变化布局 §11.9）列为待用户决定；规格仍未实施、未授权实施。** 本文把既有已确认的“手工日计划 / 已确认周计划 → 固定 Word 模板导出”规则收敛为可逐片授权实施的 I5 规格；只做规格、现状核对与实施切片设计，不含任何业务代码、schema/迁移、依赖安装或服务启动。
...
- **日计划**：客户端**不提交**内容版本——导出的“明确版本”由服务端在该请求的只读事务内读取每份计划的 `current_content_id/version` 并钉住，生成与读取同请求完成，**同一请求内**不存在“预检与下载分属两次请求”的版本混读，也就不存在伪造 daily 版本的通道。这不等于缺项确认流没有跨请求问题：409 提示与 `ack_missing=true` 重试是两次请求，其间确认对象是否允许变化不由版本钉住解决，见 §3.5 与 §11.8。响应头 `X-Export-Daily-Plan-Versions`（工程方案，可省略）可回传钉住结果。
...
- **跨请求确认对象是否允许变化：待用户决定（§11.8）。** 案例：首次 409 提示缺活动目标，期间另一教师保存新版本且活动过程也变空，用户再以 `ack_missing=true` 重试——服务端当次重算的 `facts` 只能证明清单是最新计算，**不能证明用户看过新增缺项**，不得把“每次重算”写成已确认的防绕过保证。既有确认依据已核对：[日计划 Contract](../modules/daily-plans.md) 保存与删除契约与[最小实施规格](manual-plans-and-word-export.md) 步骤 2 只规定“先提示、明确确认后仍可导出”，未规定确认绑定已展示的版本/事实还是覆盖重试时最新内容；候选 A/B、用户可见差异与最晚决定时点见 §11.8。版本清单、指纹、确认令牌等只是实现候选的工程机制，本轮不新增表、不实现。
...
服务端权威判定统一复用 I4 §7.1 的逐条比较键（`daily_plan_id + content_id + content_version`，条目级加 `group_id + game_id`），把比较对象换成**本次实际钉住的确认行 R** 的 content，即得 R 的来源新旧；版本维度比较 `R.version`（当前确认指针版本）与 `R.draft_version`（当前草稿版本）。
...
| 2 | `R.draft_version` < 当前草稿版本 | 确认后又有新草稿 | `confirmed_not_latest`（原因 `draft_ahead`） |
| 3 | 逐条来源不等（如已确认 V1 后日计划保存 V2、投影尚未刷新） | R 的内容未包含最新来源 | `confirmed_not_latest`（原因 `stale_sources`）——既有“导出确认 V1 提示未更新”要求的落实，不是新问题 |
| 4 | `R.version` < 当前确认指针版本（显式导出历史确认版本） | R 不是最新确认 | `confirmed_not_latest`（原因 `superseded`） |
...
| 层 | 覆盖内容 |
| --- | --- |
| 纯单元测试（无 DB、无 HTTP、无外部进程） | 单日/按周/按月/自选范围筛选与严格边界；周相交（周末上课、六/七教学日、假期、单日选整周、跨月跨学期、零教学日周、范围仅命中非上课日）；去重与升序（日期 / **实际周起始日期**；跨学期末周与首周同范围 → 按实际周起始日期升序（前学期末周在前、后学期首周在后）、不同学期相同周号不合并、同一实际起始日跨学期并存时按最小教学日排学期先后、整周不裁切）；无匹配判定；版本钉住与模式互斥；防伪造（伪造 `plan_id`/`confirmed_version`/夹带 `class_id` 的分支）；权限矩阵（mock 依赖）；409 ack 门与 `facts` 计算（跨请求分支见表下“待定项分支预期”）；缺项判定口径；确认快照 `effective` 输出（`no_plan`+非空 override 保留人工文本；`no_plan`+空 `effective` 留空且不标假期；`saved`+显式空字符串 override 留空且不被 `source` 补回）；§3.6 警示判定表五情形；表头起止与周次（**断言不从列数推算**）；字段映射 fixtures（W1/W6 型数据）；标红 diff 文字预期表（W2 型夹具：新增/替换/纯删除/纯移动/仅格式/无基准）；文件名生成 |
...
| 浏览器下载流程检查（API + Vite + MySQL + Chrome，需授权） | 导出按钮可见性与权限（同班只读教师可用、跨班不可见/报错）；缺项 409 提示 → 明确确认 → 下载成功（重试期间内容变化的跨请求分支按 §11.8 届时决定补充执行）；无匹配提示且无文件；“未包含最新变化”与“无法比较”提示可见（服务端承载 + 前端展示，§3.6）；下载文件名/类型正确 |
| Microsoft Word | **既有 contract：正式使用后反馈项**，不列入当前完成门槛，不声称已验收 |

#### 待定项分支预期（对应 §11.8 / §11.9）

**在用户决定前不作为已确认验收标准**；决定后按所选项把对应分支补入上表相应层。任一分支下，“每次请求重新鉴权、服务端独立计算 `facts`、不信任客户端 facts”均为固定通过标准；不得把“服务端仅重算 facts 即通过”写成已确认验收。

| 待定项 | 案例 | 选项 A 预期 | 选项 B 预期 |
| --- | --- | --- | --- |
| §11.8 缺项确认跨请求绑定 | 首次 409（缺活动目标）→ 另一人保存新版本且活动过程也变空 → `ack_missing=true` 重试 | 重试成功导出、缺项保持为空；服务端当次记录的 `facts` 含新增缺项（确认 = 覆盖重试时最新内容及其全部缺项） | 再次 409 并提示新增缺项，用户再次确认后才导出（确认绑定首次已展示的版本/事实） |
| §11.9 确认后日历变化布局 | 确认后周日教学→休息；休息日→教学；学期边界变化 | 按届时选择分别定义列集合、表头与内容处理后补入对应预期 | 同左（两选项均为待定，此处不预设结果） |
...
| 8 | 日计划缺项确认的跨请求绑定（§3.3/§3.5） | A：**宽泛确认**——`ack_missing=true` 覆盖重试时的最新内容及其全部缺项，以服务端当次重算为准（实现最简；用户可能未看到期间新增的缺项）；B：**绑定已展示的版本/事实**——确认对象是首次 409 展示的版本与缺项清单，期间相关字段变化时重新 409 提示（确认对象一致；版本清单/指纹/确认令牌等仅是候选工程机制，不新增表、不实现） | 建议优先评估 B（保留确认对象一致性）；**尚未确认** | 片 1 的 ack 门逻辑定型前（**片 1 依赖**） |
...
以上均未确认；实施切片中涉及对应项时必须先取得用户决定。**片 1 真正依赖的决定是 §11.7 与 §11.8**；§11.9 不阻塞片 1–4（场景当前不可发生）；§11.1–§11.6 留在各自原时点（片 2/3），不要求现在一次性批准。技术上可由既有架构直接推出、且不改变产品行为的部分已按“工程方案”给出，不列为产品问题；用户已确认的行为（如“导出确认 V1 提示未更新”）只落实、不重新提交审批。
...
- **完成条件**：§10.2 纯单元行全绿；权限、版本钉住、相交/去重/排序（含跨学期按实际周起始日期排序、按计划身份去重）、缺项与 409 facts、`no_plan`/`effective` 输出、§3.6 警示判定表五情形、表头/周次、映射与标红 diff 均有断言；§11.8 待定分支在其决定后补入，未决定前不把任一分支写成通过标准；现有 I3/I4 单测回归通过。
...
- **完成条件**：路由/权限单测（mock）覆盖 §8 全表；隔离 MySQL 集成测试通过（权限矩阵、并发一致快照、审计记录、零业务写入）；前端 typecheck/build 通过；无匹配/缺项/警示交互闭环（警示由服务端承载、前端必须展示，见 §3.6；409 重试的跨请求分支按 §11.8 届时决定执行）。
...
- 契约空白 3（2026-09-25 定向修订新增，**不改 Contract**）：日计划缺项确认的跨请求绑定（§3.5）与确认后日历变化的导出布局（§4.2/§5.3）未被既有 Contract 唯一确定 → §11.8、§11.9 待用户决定；“导出确认 V1 提示未更新”等既有确认行为只落实不重问。
...
- **2026-09-25 定向修订**：修正 §5.2 `no_plan` 输出、§4.2 跨学期排序与去重键、§3.6 警示判定与承载，并把 §3.5/§11.8 与 §4.2/§11.9 两项语义写成待用户决定；修订只涉及本文与 readiness 状态句，未改 Contract、ARCHITECTURE、AGENTS、产品代码、依赖、schema 或迁移。
