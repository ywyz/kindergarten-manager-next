...
状态：**I4 规格已确认（本范围内，U1–U5 五项产品决定已于 2026-09-23 确认，见 §1.5）；代码、迁移、测试与前端实施未获授权。** 本规格基于 main / 1e6e3a6 的定向只读核对编写；本轮只写文档，未写业务代码、迁移、测试或前端，未启动 API、Vite、数据库或容器，未安装依赖，未执行迁移或测试，未暂存、提交或推送。
...
未来获授权后按片申请；**不默认沿用 I3 已结束且容器已删除的任何资源**：

1. 隔离 MySQL 8.4 / InnoDB：I4 专用一次性容器（如 `kg-next-i4-mysql-*`，仅 `127.0.0.1` 高位端口）与白名单库 `kindergarten_test_i4`／`kindergarten_test_i4_fresh`，guard 拒绝 I1/I2/I3／共享／生产库；不读 `.env`。
2. 迁移：`APP_DISABLE_DOTENV=1` + 显式 I4 DSN 下 `alembic upgrade head`；凭据不写入文档。
3. 依赖：当前无新增 Python／Node 依赖预期。
4. 测试：`unittest` 纯单元与 `tests.integration.test_i4_*`（需显式授权的破坏性白名单开关）；skip 不算通过。
5. 浏览器：临时 uvicorn + Vite 供 main 验证，结束后停止、删除容器。
6. 本轮不执行以上任何一项，不暂存、提交、推送或部署。

## 结束说明

本规格在本范围内**已确认**（U1=A、U2=B、U3=A、U4 双路径、U5=A，2026-09-23）：schema、API、锁协议、确认事实与校验条款可按本文冻结，无遗留待选项（§1.5 表中被拒方案仅作决策留痕）。**代码、迁移、测试、前端与部署仍未授权**；开工须用户按第 10 节逐片授权。本轮未运行测试、迁移、服务或数据库，未实施 I4，未暂存、提交或推送。
