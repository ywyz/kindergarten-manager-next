...
### 12.3 I4 复审后的两处窄修复（F1/F2，2026-09-24）

第四片验收与 §12.2 最终窄修复的复审（`docs/bootstrap/i4-latest-review-2026-09-24.md`，基线 `3120a2c`）发现两处残留问题，本轮按定向提示词完成修复：

- **F1（P1）根因**：`doRefresh()` 成功后无条件把 `editBaseVersion` 推进到刷新生成的新版本。刷新接口只按请求携带的 `expected_draft_version` 校验并递增草稿版本，既不提交也不解决本地 dirty 输入与他人编辑的冲突——刷新成功不是旧编辑冲突已解决的证据。于是“确认冲突复核 v2 → 取消 → 以展示版本刷新生成 v3 → 保存”会以 v3 无条件 PATCH，覆盖他人编辑而不触发保存侧 409。
- **F2（P2）根因**：确认对话框新增的目标预览对户外游戏只渲染名称（加“手工补充”标记）、对重点区域只渲染名称；同名但目标/指导/来源版本不同的已保存确认目标无法在确认前完整复核，且日期与来源标识不可见。
- **修复范围**：仅 `frontend/src/views/WeeklyPlanView.vue` 与 `frontend/src/weekly-plan-content.ts`（后者新增两个纯展示 helper：`outdoorSlotSnapshotPreview`、`focusAreaSnapshotPreview`，不复用依赖 `form` 的 `focusTexts()`/`focusMeta()`）。后端 service/API/schema/迁移/权限/锁协议、U1–U5、`TeacherView.vue`、`ARCHITECTURE.md` 均 0 修改；未新增依赖。
- **F1 行为**：刷新成功后仅在“无 dirty 输入”或“dirty 真实基线 == 本次刷新目标版本”时推进 `editBaseVersion`（后者意味着版本推进只来自用户自己同基线的显式刷新）；否则保留真实基线、不静默重基，并进入既有显式冲突处理（新增 reason `refresh_kept_baseline`，仍是“放弃本地 / 以服务端为基准重提”两种处理，保存在冲突期间保持阻断）。刷新本身照常完成且不自动 PATCH/confirm、不丢输入；正常同基线刷新与无 dirty 刷新行为不变，不存在“永久禁用刷新”的修复。
- **F2 行为**：确认目标的游戏/重点区域预览改由读取 `confirmTarget.content` 的 `confirmSlotPreviews`/`confirmFocusPreview` computed 渲染——户外游戏逐项显示名称、来源类型（日计划来源/手工补充）、共用目标/指导要点/重点指导；重点区域显示名称、上下文（区域/户外/专用室，区域空值可辨）与已存目标/指导/支持策略；空值一律显示“（空）”不隐藏差异；来源行展示可靠映射的日期（存储引用在当前候选中能解析才显示，映射不上仅省略、绝不虚构）+ 内容版本 + 来源标识（日计划为 `content_id`，手工为 `manual_item_id`），可区分同名不同来源；快照文本不被最新来源候选替换，日期仅用于映射展示。

实际执行检查（2026-09-24，均为本轮真实运行）：

- **F1 脚本级定向回归**（复用既有 TypeScript 转译 + Vue 运行时；API 受控桩，刷新桩与后端契约一致：只校验 expected、递增版本、不接收 theme/dirty 输入）：A 复现路径（v1 dirty → 确认 409 → 复核 v2 → 取消 → 刷新 → 保存）与 B/C/D/E/F 场景共 **29 项全部通过**——刷新生成 v3 后基线保持 v1 并进入显式冲突；冲突未处理时保存被阻断、服务端主题不被覆盖；直发旧基线 PATCH 仍 409；显式“以服务端为基准重提/放弃本地”两路径正确收口；同基线刷新（dirty 保留、安全推进 v1→v2）、无 dirty 刷新、刷新 409→重提刷新不重基、保存 409 两路径，以及 ACK_REQUIRED 更新最新 facts/重置 ack/不自动重试、确认成功保留 dirty、服务器超前进入显式冲突等既有行为全部保持。同一套断言在修复前组件（`git show 3120a2c`）上 **11 项失败**，确认回归非空转。
- **F2 helper 夹具 + 模板结构检查 22 项全部通过**：同名但目标/指导不同的手工游戏、同名不同来源（不同日期/版本/标识）的日计划游戏、重点区域三种上下文与空值/日期映射夹具；确认对话框模板区域不再引用 `form`/`focusTexts()`/`focusMeta()`，预览数据源为 `confirmTarget.content`。
- **F2 真实模板渲染（vnode 文本层）18 项全部通过**：用已安装的 `vue/compiler-sfc` 编译真实 SFC（script setup + template），以已安装的 `vue` 运行时执行编译出的真实 `render()` 生成 vnode 树（Element Plus 以同名桩组件呈现并展开插槽文本；未启动服务、未访问浏览器、无新依赖），断言确认对话框子树渲染文本：主题/游戏/区域文本来自 `confirmTarget.content` 快照（本地 dirty 主题“本地未保存主题”不出现在对话框文本中）、名称+来源类型+三项整组文本+上下文完整可见、`09-21 · 内容 v2 · 标识 ca` 与 `09-22 · 内容 v3 · 标识 fx` 等来源行可见、空值“（空）”、空槽位“未选择”、快照不被最新候选替换。
- `npm run typecheck` 与 `npm run build` 通过（build 仅有既有 bundle 体积警告）；`git diff --check` 通过；本次业务代码改动仅上述两个前端文件（同工作区内的状态文档更新属于记录，非业务代码改动）。
- **未执行**：真实浏览器 DOM/交互验证（本任务边界内不允许启动服务、数据库或浏览器环境，也未新增依赖）——脚本级与 vnode 级检查不等同于浏览器验证，F1 冲突提示文案与 F2 预览的浏览器实机呈现留给 main 在明确授权的隔离环境做定向验证；§8 的 54 项浏览器矩阵、MySQL V1–V14、后端全套测试本轮未重跑（后端 0 修改且未触及其覆盖范围）。
